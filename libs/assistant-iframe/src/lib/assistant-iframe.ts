...
export class AssistantIframe {
  ...
  onLoad(): void {
    ...
    this.iframe.addEventListener( 'load', () => {
      const currentState = this.iframe?.contentDocument?.body.classList.contains(this.config.bodyOpenClass);
  ...
  watchToggleButton(): void {
    const iframeButtons = this.iframe?.contentDocument?.getElementsByClassName(this.config.toggleClass);
